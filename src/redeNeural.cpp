...
void redeNeural_task(void* pvParameters) {
    ...
    float layer1_a[20];
    float layer2_z[8];
    float probs_unnorm[8];  // Probabilidade com Prior (sem dividir pela soma)
    ...
    for (;;) {
        ...
        float leituraAtual[4];  // Copia local
        ...
        float max_z = -99999.0;  // Para estabilidade do Softmax
        for (int k = 0; k < 8; k++) {
            float z = 0.0;
            for (int j = 0; j < 20; j++) {
        ...
        for (int k = 0; k < 8; k++) {
            float exps = exp(layer2_z[k] - max_z);
        ...
        for (int c = 0; c < 8; c++) {
            ...
            for (int k = 0; k < 8; k++) {
                currentRisk += probs_unnorm[k] * (float)fCost[k][c];
            }
